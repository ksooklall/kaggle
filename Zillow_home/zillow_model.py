...
def clean_columns(df):
    ...
    id_col = [col for col in df.columns if ('id' in col) and (col != 'parcelid')]
...
#df = filter_outliers(df, 'logerror')
df = filter_cols_by_nan(df, 0.98)
trdf = train_df.merge(df, how='left', on='parcelid')
trdf = trdf[~trdf['parcelid'].duplicated()]

tedf = test_df.merge(df, how='left', on='parcelid')
tedf = tedf[~tedf['parcelid'].duplicated()]
# Get train_df and test_df

import pdb; pdb.set_trace()
train_cols = set([col.split('_')[0] for col in trdf.columns if '20' not in col])
train_df = filter_outliers(trdf, 'logerror')
...
model, model_history = vanilla_nn(X_train, y_train, 1024, 0.3, 256, depth=4)
