...
from KerasUtilities import plot_model, vanilla_nn
...
re_sampling_df = up_sampling(full_train_df, SEED)
X, y, = get_training_testing(full_train_df[train_cols], one_hot=True)
...
def predict_and_submit(model):
    x_test = np.array(test_df[[i for i in train_cols if i not in ['target', 'id']]])
    y_pred = model.predict(x_test)
    submission = pd.DataFrame({'id': test_df['id'], 'target': y_pred[:, 1]})
    submission.to_csv('submissions/submission_{}.csv'.format(10), index=False)

model, model_info = vanilla_nn(X_train, y_train, hidden_units, drop_out, batch_size,
                               depth=4, loss='categorical_crossentropy', filepath='vnn.weights.h5')
